...
module.exports = class Database {
    ...
    open(filename, mode) {
        ...
        return new Promise((resolve, reject) => {
            if (this.db) {
                return reject(
                    new Error("Database.open: database is already open")
                );
            }
    ...
    close(fn) {
        if (!this.db) {
            return Promise.reject(
                new Error("Database.close: database is not open")
            );
        }
        if (fn) {
            return fn(this)
    ...
    prepare(...args) {
        return new Promise((resolve, reject) => {
            if (!this.db) {
                return reject(
                    new Error("Database.prepare: database is not open")
                );
            }
            let statement;
            let callback = (err) => {
...
class Statement {
    constructor(statement) {
        if (!(statement instanceof sqlite.Statement)) {
            throw new TypeError(
                `Statement: 'statement' is not a statement instance`
            );
        }
